...
`diagnostics.validate` is editor-agnostic — it can wrap anything that returns markers, including a JSON Schema validator. This example validates a ledger schema (YAML) against the canonical [ledger schema](https://ds.formance.com/schemas/ledger-schema) hosted by this design system: it fetches the schema, compiles it with [Ajv](https://ajv.js.org), and maps each error to a diagnostic. The `.description` on the client account is accepted, while the invalid `resource` on `CLIENT_BALANCE` (it must be one of `transactions`, `accounts`, `logs`, `volumes`) is flagged.
